<script lang="ts">
	...
	let aiGraded = $derived(filteredGrades.filter((g) => g.gradedBy === 'ai').length);
	let manualGraded = $derived(filteredGrades.filter((g) => g.gradedBy === 'manual').length);
...
<div class="space-y-6">
	...
	<div class="flex items-start justify-between">
		<div>
			...
			<p class="mt-1 text-gray-600">View and manage student grades across all assignments</p>
		</div>
		<button
		...
		<div class="grid grid-cols-1 gap-6 md:grid-cols-4">
			...
			<div class="rounded-lg bg-white p-6 shadow">
				<div class="flex items-center">
					...
					<div class="ml-4">
						<p class="text-sm font-medium text-gray-600">AI Graded</p>
						<p class="text-2xl font-bold text-gray-900">{aiGraded}</p>
					</div>
				</div>
			</div>
			...
			<div class="rounded-lg bg-white p-6 shadow">
				<div class="flex items-center">
					...
					<div class="ml-4">
						<p class="text-sm font-medium text-gray-600">Manual Graded</p>
						<p class="text-2xl font-bold text-gray-900">{manualGraded}</p>
					</div>
				</div>
			</div>
		</div>

		<!-- Filters and Grade List -->
		<div class="rounded-lg bg-white shadow">
			<div class="border-b border-gray-200 p-6">
				<div class="flex items-center space-x-4">
					<label for="assignment-filter" class="text-sm font-medium text-gray-700">
				...
				{#if filteredGrades.length === 0}
					<div class="p-12 text-center">
						...
						<p class="mt-1 text-sm text-gray-500">
							No grades available for the selected assignment(s).
						</p>
					</div>
				{:else}
							...
							{#each filteredGrades as grade, index (grade.id || `grade-${index}-${grade.assignmentId || 'unknown'}-${grade.studentId || 'unknown'}`)}
								...
								<tr class="hover:bg-gray-50">
									...
									<td class="px-6 py-4 text-sm font-medium whitespace-nowrap">
										<button class="mr-3 text-blue-600 hover:text-blue-900" title="View feedback">
											View
										</button>
										<button class="text-gray-600 hover:text-gray-900" title="Edit grade">
											Edit
										</button>
									</td>
								</tr>
