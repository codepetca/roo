...
class DataStore {
  ...
  // Computed state
  hasData = $derived(
    this.classrooms.count > 0 || this.assignments.count > 0
  );
  ...
  selectedClassroom = $derived(
    this.selectedClassroomId 
      ? this.classrooms.get(this.selectedClassroomId) 
      : null
  );
  
  selectedAssignment = $derived(
    this.selectedAssignmentId 
      ? this.assignments.get(this.selectedAssignmentId) 
      : null
  );
  ...
  async loadDashboardData(): Promise<void> {
    try {
      ...
      if (dashboardData.classrooms && dashboardData.classrooms.length > 0) {
        ...
        console.log('🏠 Loaded classrooms:', classroomModels.length);
        
        // Load assignments for these classrooms
        await this.loadAssignmentsForClassrooms(
          classroomModels.map(c => c.id)
        );
      }
  ...
  setupRealtimeListeners(): void {
    ...
    realtimeService.subscribeToClassrooms(
      this.currentUser.schoolEmail,
      this.classrooms
    );
    ...
    if (classroomIds.length > 0) {
      realtimeService.subscribeToAssignments(
        classroomIds,
        this.assignments
      );
    }
