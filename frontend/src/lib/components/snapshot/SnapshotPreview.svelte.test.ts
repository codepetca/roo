...
describe('SnapshotPreview Component', () => {
  ...
  describe('Classrooms Preview', () => {
    it('should render classrooms list with details', async () => {
      ...
      expect(screen.getByText(`Classrooms (${testSnapshot.classrooms.length})`)).toBeInTheDocument();
      ...
      expect(screen.getByText(new RegExp(`${firstClassroom.studentCount} students`))).toBeInTheDocument();
      expect(screen.getByText(new RegExp(`${firstClassroom.assignments.length} assignments`))).toBeInTheDocument();
      expect(screen.getByText(new RegExp(`${firstClassroom.submissions.length} submissions`))).toBeInTheDocument();
    });
  ...
  describe('Snapshot Metadata', () => {
    it('should render snapshot metadata correctly', async () => {
      ...
      expect(screen.getByText(new Date(testSnapshot.snapshotMetadata.fetchedAt).toLocaleString())).toBeInTheDocument();
      expect(screen.getByText(new Date(testSnapshot.snapshotMetadata.expiresAt).toLocaleString())).toBeInTheDocument();
