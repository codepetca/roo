<script lang="ts">
	...
	const pendingRate =
		totalSubmissions > 0 ? Math.round((pendingSubmissions / totalSubmissions) * 100) : 0;
...
<Card>
	{#snippet children()}
		<div class="p-6">
			<div class="flex items-start justify-between">
				<div class="flex-1">
					...
					{#if assignment.description}
						<p class="mt-1 line-clamp-2 text-sm text-gray-600">{assignment.description}</p>
					{/if}
			...
			{:else}
				<div class="mt-4 py-4 text-center text-sm text-gray-500">No submissions yet</div>
			{/if}
