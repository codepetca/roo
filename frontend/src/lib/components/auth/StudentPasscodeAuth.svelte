<script lang="ts">
	...
	async function handleRequestPasscode() {
		...
		try {
			...
			success = `Login code sent to ${email}. Check your email and enter the 6-digit code below.`;
			...
			if (err.message?.includes('Teacher authentication required')) {
				...
				error = err.message || 'Failed to send login code. Please ask your teacher to send you a login code.';
			}
		} finally {
			loading = false;
		}
	...
	{:else}
		...
		<div class="space-y-4">
			<div class="text-center">
				...
				<p class="text-sm text-gray-600">
					Check your email for a 6-digit login code from your teacher.
				</p>
			</div>
			...
			<Button onclick={handleVerifyPasscode} disabled={loading || passcode.length !== 6} class="w-full">
				{#if loading}
					<svg
						class="mr-3 -ml-1 h-5 w-5 animate-spin text-white"
						xmlns="http://www.w3.org/2000/svg"
