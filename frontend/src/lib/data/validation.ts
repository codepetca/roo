 ...
 */
export const callableResultSchema = <T extends z.ZodType>(dataSchema: T) => z.object({
	data: dataSchema
});
 ...
 */
export function validateApiResponse<T>(dataSchema: z.ZodSchema<T>, response: unknown): {
	success: boolean;
	data: T;
	error?: string;
	message?: string;
