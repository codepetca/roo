...
export class AssignmentCollection extends BaseCollection<AssignmentModel> {
   ...
   */
  quizzes = $derived(
    this.filter(assignment => assignment.isQuiz)
  );
   ...
   */
  withUngraded = $derived(
    this.filter(assignment => assignment.hasUngraded)
  );
   ...
   */
  overdue = $derived(
    this.filter(assignment => assignment.isOverdue)
  );
   ...
   */
  sortedByUngraded = $derived(
    this.sorted((a, b) => b.ungradedCount - a.ungradedCount)
  );
  
  /**
   * Get assignments sorted by title
   */
  sortedByTitle = $derived(
    this.sorted((a, b) => a.displayTitle.localeCompare(b.displayTitle))
  );
   ...
   */
  autoGradable = $derived(
    this.filter(assignment => assignment.isAutoGradable())
  );
  ...
  get statistics() {
    return $derived({
      ...
      averageCompletion: this.all.length > 0
        ? Math.round(
            this.all.reduce((sum, a) => sum + a.completionRate, 0) / this.all.length
          )
        : 0
    });
  }
