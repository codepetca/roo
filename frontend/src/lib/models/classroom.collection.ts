...
export class ClassroomCollection extends BaseCollection<ClassroomModel> {
   ...
   */
  withUngradedWork = $derived(
    this.filter(classroom => classroom.ungradedSubmissions > 0)
  );
   ...
   */
  active = $derived(
    this.filter(classroom => classroom.courseState !== 'ARCHIVED')
  );
  ...
  totalStudents = $derived(
    this.all.reduce((sum, classroom) => sum + classroom.studentCount, 0)
  );
   ...
   */
  sortedByName = $derived(
    this.sorted((a, b) => a.name.localeCompare(b.name))
  );
   ...
   */
  withStudents = $derived(
    this.filter(classroom => classroom.studentCount > 0)
  );
