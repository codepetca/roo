...
import { signInAsTeacher, verifyDashboardData, waitForPageReady, debugPage } from './test-helpers';
	...
	test('should show real imported data (not empty state)', async ({ page }) => {
		try {
			...
			if (hasRealData) {
				...
				throw new Error(
					'Dashboard does not show imported classroom data - only empty state or static UI'
				);
			}
		} catch (error) {
	...
	test('should navigate to classroom when data exists', async ({ page }) => {
		try {
			...
			const classroomView =
				...
				(await page
					.locator('text=/assignment|submission|student.*list/i')
					.isVisible({ timeout: 3000 })
					.catch(() => false));
