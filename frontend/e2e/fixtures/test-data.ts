 ...
 */
export function createMockStudents(count: number, classroomId: string = 'classroom-1'): StudentSnapshot[] {
  const firstNames = ['Alice', 'Bob', 'Carol', 'David', 'Eve', 'Frank', 'Grace', 'Henry', 'Ivy', 'Jack'];
  const lastNames = ['Anderson', 'Brown', 'Chen', 'Davis', 'Evans', 'Garcia', 'Harris', 'Johnson', 'Kumar', 'Lopez'];
 ...
 */
export function createMockAssignments(count: number, classroomId: string = 'classroom-1'): AssignmentWithStats[] {
...
export function createMockSubmissions(
  ...
  assignments.forEach(assignment => {
    ...
    const submittingStudents = students
    ...
    submittingStudents.forEach((student, index) => {
      ...
      if (isGraded) {
        ...
        submission.grade = {
          ...
          feedback: score >= assignment.maxScore * 0.9 
...
export function createMockClassroomWithData(
  ...
  return {
    ...
    creationTime: new Date(Date.now() - (30 + classroomIndex * 7) * 24 * 60 * 60 * 1000).toISOString(),
