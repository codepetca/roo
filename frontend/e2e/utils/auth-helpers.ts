...
export class AuthHelper {
  ...
  async loginViaEmailPassword(credentials = DEMO_CREDENTIALS.teacher) {
    ...
    try {
      ...
      const errorMessage = this.page.locator('[data-testid="auth-error-message"], .error, [role="alert"]').first();
  ...
  async createTeacherAccountViaEmail(credentials = DEMO_CREDENTIALS.teacher) {
    ...
    const signupToggle = this.page.locator('button:has-text("Create one")').or(
      this.page.locator('text=/create.*account/i')
    );
    ...
    const schoolEmailInput = this.page.locator('input[placeholder*="school"]').or(
      this.page.locator('input:below(:text("School Email"))')
    );
    ...
    const createButton = this.page.locator('button:has-text("Create Account")').or(
