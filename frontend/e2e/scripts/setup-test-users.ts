...
const FIREBASE_PROJECT_ID =
	...
	(TEST_ENVIRONMENT === 'staging' ? 'roo-staging-602dd' : 'roo-app-3d24e');
const FIREBASE_ADMIN_SDK_PATH =
	process.env.FIREBASE_ADMIN_SDK_PATH || '../../../service-account-key.json';
...
function initializeFirebaseAdmin() {
	try {
		...
		if (USE_EMULATORS) {
			...
			console.log(
				`✅ Initialized Firebase Admin SDK for ${TEST_ENVIRONMENT.toUpperCase()} environment`
			);
			console.log(`   Project: ${FIREBASE_PROJECT_ID}`);
		}
		...
		console.error(
			`❌ Failed to initialize Firebase Admin SDK for ${TEST_ENVIRONMENT}:`,
			error.message
		);
...
async function createAllTestUsers(): Promise<void> {
	console.log(
		`🚀 Starting test user creation for ${TEST_ENVIRONMENT.toUpperCase()} environment...\n`
	);
...
function verifyEnvironment(): void {
	...
	const requiredEnvVars = ['FIREBASE_PROJECT_ID', 'FIREBASE_ADMIN_SDK_PATH'];
