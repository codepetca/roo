...
export function getStudentsForTeacher(teacherKey: keyof typeof TEST_TEACHERS): TestUserConfig[] {
  ...
  return enrolledStudentKeys.map(studentKey => 
    TEST_STUDENTS[studentKey as keyof typeof TEST_STUDENTS]
  ).filter(Boolean);
}
...
export function getTeachersForStudent(studentKey: keyof typeof TEST_STUDENTS): TestUserConfig[] {
  ...
  return uniqueTeacherKeys.map(teacherKey => 
    TEST_TEACHERS[teacherKey as keyof typeof TEST_TEACHERS]
  ).filter(Boolean);
}