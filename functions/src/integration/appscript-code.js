...
function doGet(e) {
  ...
  if (!spreadsheetId) {
    return ContentService.createTextOutput(
      "Error: spreadsheetId parameter is required. Usage: ?spreadsheetId=YOUR_SHEET_ID"
  ...
  try {
    ...
    return ContentService.createTextOutput("Success: Processed all submissions and wrote to spreadsheet").setMimeType(
      ContentService.MimeType.TEXT
    );
  } catch (error) {
    console.error("Error in doGet:", error);
    return ContentService.createTextOutput("Error processing submissions: " + error.toString()).setMimeType(
      ContentService.MimeType.TEXT
    );
  }
}
...
function processAllSubmissions() {
  ...
  console.log("\n========================================");
  console.log("SYNCING TO ROO SYSTEM");
  console.log("========================================");
  try {
    const syncResult = syncToRooSystem(CONFIG.PERSONAL_SPREADSHEET_ID, getStoredApiKey());
    if (syncResult) {
      console.log("✅ Roo sync completed successfully");
    } else {
      console.log("⚠️ Roo sync had issues - check logs");
    }
  } catch (error) {
    console.log("❌ Roo sync failed:", error.toString());
    // Don't throw - let the main process complete even if sync fails
  }
}
 ...
 */
function extractSubmissionsFromResponseSheet(fileId, assignmentTitle, courseId, createdDate, fileName, isQuiz, formId) {
  isQuiz = isQuiz || false;
  formId = formId || null;
  var submissions = [];
  ...
  try {
    ...
    throw new Error(
      "Spreadsheet access failed: " +
        error.toString() +
        ". Please ensure the spreadsheet exists and you have edit permissions."
    );
  }
}
...
function recordLastSync() {
  const properties = PropertiesService.getScriptProperties();
  properties.setProperty("ROO_LAST_SYNC_TIME", new Date().toISOString());
}
