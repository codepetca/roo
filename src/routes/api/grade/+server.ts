...
export const POST: RequestHandler = async ({ request }) => {
  try {
    ...
    if (uploadError) {
      const errorResponse: APIResponse = {
        success: false,
        error: { message: "Failed to upload image", code: uploadError.name },
      };
    ...
    if (questionError || !question) {
      const errorResponse: APIResponse = {
        success: false,
        error: {
          message: "Question not found or error fetching it",
    ...
    if (typeof question.rubric === "string") {
      try {
        ...
        const errorResponse: APIResponse = {
          success: false,
          error: { message: "Failed to parse question rubric" },
        };
        return json(errorResponse, { status: 500 });
      }
    } else {
    ...
    const response: APIResponse<Submission> = {
      success: true,
